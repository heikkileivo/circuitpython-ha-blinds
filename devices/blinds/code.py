...
# The health task feeds the watchdog and checks the escalation this often.
HEALTH_CHECK_S = 1
# A restart the firmware triggers deep-sleeps this long.
RESTART_SLEEP_S = 1
# After a failed run of main(), the next one starts this long later.
RETRY_S = 10
# The escalation window: the blind restarts once its liveness echo has been
# missing this long. A run of main() that lasts longer resets the restart
# loop's count.
ESCALATION_MS = os.getenv("mqtt_escalation_s", recovery.WINDOW_MS // 1000) * 1000
...
async def watch_health(mqtt, blinds, escalation, wifi_up):
    """
    Feed the watchdog, and restart once the liveness echo has been missing
    for the escalation window, as soon as the blind isn't moving.
    ...
    ends in a reset, 16 s later: the radio gives up reconnecting by itself,
    and connect_wifi() only runs at boot.
    """
    ...
    while True:
        ...
        if escalation.due(t_ms, blinds.in_move):
            print(f"MQTT escalation: no liveness echo for {ESCALATION_MS // 1000} s, restarting.")
            reset_cause.restart(reset_cause.MQTT_ESCALATION, RESTART_SLEEP_S)
        if blinds.is_moving or wifi.radio.connected or not wifi_up.is_set():
            wifi_lost = False
            microcontroller.watchdog.feed()
        elif not wifi_lost:
            wifi_lost = True
...
async def run_blind(reader):
    ...
    # retries too.
    wifi_up = asyncio.Event()
    escalation = recovery.Escalation(now_ms(), ESCALATION_MS)
    tasks = [asyncio.create_task(watch_health(mqtt, blinds, escalation, wifi_up))]
...
# main() only ends by failing. It runs again RETRY_S later, until it fails
...
while True:
    ...
    gc.collect()
    print(f"Running main() again in {RETRY_S} s...")
